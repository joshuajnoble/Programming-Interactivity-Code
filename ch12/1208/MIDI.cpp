...
void MIDI_Class::send(kMIDIType type, byte data1, byte data2, byte channel) {
	...
	if (type <= PitchBend) {
		...
		if (mRunningStatus_TX != statusbyte) {
			...
			USE_SERIAL_PORT.print(mRunningStatus_TX);
		}
#else
		// Don't care about running status, send the Control byte.
		USE_SERIAL_PORT.print(statusbyte);
#endif
		
		// Then send data
		USE_SERIAL_PORT.print(data1);
		if (type != ProgramChange && type != AfterTouchChannel) {
			USE_SERIAL_PORT.print(data2);
		}
		return;
	}
...
/*! \brief Send a Pitch Bend message using a signed integer value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_Class::sendPitchBend(int PitchValue,byte Channel) {
...
void MIDI_Class::sendSysEx(byte length, byte * array, bool ArrayContainsBoundaries) {
	if (!ArrayContainsBoundaries) USE_SERIAL_PORT.print(0xF0);
	for (byte i=0;i<length;i++) USE_SERIAL_PORT.print(array[i]);
	if (!ArrayContainsBoundaries) USE_SERIAL_PORT.print(0xF7);
#if USE_RUNNING_STATUS
	mRunningStatus_TX = InvalidType;
#endif
}
...
void MIDI_Class::sendTimeCodeQuarterFrame(byte data) {
	...
	USE_SERIAL_PORT.print((byte)TimeCodeQuarterFrame);
	USE_SERIAL_PORT.print(data);
#if USE_RUNNING_STATUS
	mRunningStatus_TX = InvalidType;
#endif
}
...
void MIDI_Class::sendSongPosition(unsigned int Beats) {
	...
	USE_SERIAL_PORT.print((byte)SongPosition);
	USE_SERIAL_PORT.print(Beats & 0x7F);
	USE_SERIAL_PORT.print((Beats >> 7) & 0x7F);
#if USE_RUNNING_STATUS
	mRunningStatus_TX = InvalidType;
#endif
}
...
void MIDI_Class::sendSongSelect(byte SongNumber) {
	...
	USE_SERIAL_PORT.print((byte)SongSelect);
	USE_SERIAL_PORT.print(SongNumber & 0x7F);
#if USE_RUNNING_STATUS
	mRunningStatus_TX = InvalidType;
#endif
}
...
void MIDI_Class::sendRealTime(kMIDIType Type) {
	switch (Type) {
		...
		case ActiveSensing:
		case SystemReset:
			USE_SERIAL_PORT.print((byte)Type);
			break;
		default:
